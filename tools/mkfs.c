 ...
 * Copyright (c) 2008 Warren Toomey
 */
...
#define N_PROC 16   /* Hard coded in V1 kernel source */
#define RF_NOSWAPSIZE (RF_SIZE - KERNBLKS - (SWPSIZ * N_PROC))
...
void add_devdir(void)
{
    struct dev {
        ...
        {"tty2", 16}, {"tty3", 17}, {"tty4", 18}, {"tty5", 19}, {"tty6", 20},
...
int create_file(struct directory *d, char *name, int size, struct fileperm *p)
{
    ...
    if (p) {
        ...
        inodelist[inum].flags |=
            I_MODFILE | I_EXEC | I_UREAD | I_UWRITE | I_OREAD | I_OWRITE;
    }
...
void read_permsfile(char *file)
{
    ...
    while (1) {
        ...
        if (linebuf[0] == 'x')
            permlist[posn].flags |= I_EXEC;
        if (linebuf[0] == 'u')
            permlist[posn].flags |= I_SETUID | I_EXEC;
        if (linebuf[1] == 'r')
            permlist[posn].flags |= I_UREAD;
        if (linebuf[2] == 'w')
            permlist[posn].flags |= I_UWRITE;
        if (linebuf[3] == 'r')
            permlist[posn].flags |= I_OREAD;
        if (linebuf[4] == 'w')
            permlist[posn].flags |= I_OWRITE;
...
void usage(void)
{
    ...
    printf(
        "\t-p reads the named file to obtain a list of V1 file permissions\n");
    exit(1);
}
...
int main(int argc, char *argv[])
{
    ...
    if (argc != 3)
        usage();
    if (strcmp(argv[2], "rk") && strcmp(argv[2], "rf"))
        usage();
