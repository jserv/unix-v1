...
struct symtype {
    ...
    {"%d", 1, isym},        {"func%d", 1, isym},    {"data%d", 1, dsym},
    {"jsrtext%d", 1, isym}, {"jsrdata%d", 1, isym},
};

/* Debug code */
void print_symtables(void)
{
    for (int i = 0; i < PDP_MEM_SIZE; i++) {
        if (isym[i] != NULL)
            printf("0%06o %d %d %s\n", i, isym[i]->type, isym[i]->size,
                   isym[i]->name);
        if (dsym[i] != NULL)
            printf("0%06o %d %d %s\n", i, dsym[i]->type, dsym[i]->size,
                   dsym[i]->name);
    }
}
...
void add_symbol(int addr, int type, int size)
{
    ...
    if ((symtypelist[type].table[addr] != NULL) &&
        (symtypelist[type].table[addr]->type >= type))
        return;
...
void patch_symbols(void)
{
    ...
    for (i = 0; i < PDP_MEM_SIZE; i++) {
        ...
        type = s->type;
        s->name = malloc(12);
        snprintf(s->name, 12, symtypelist[type].format,
                 symtypelist[type].counter++);
    }
    for (i = 0; i < PDP_MEM_SIZE; i++) {
        if (dsym[i] == NULL)
            continue;
